   ...
   <searchbox>
     ...
     <view class="search_info">
       <repeat for="{{storageList}}" key="index" index="index" item="item">
        <view class="info_box">
        <navigator url="./pigdetails?id= {{item.id}}">
...
export default class BuyPig extends wepy.page {
  config = {
    enablePullDownRefresh: true,
    navigationBarTitleText: "买猪"
  };
  data = {
    buyList: [],
    storageList:[],
  ...
  async getBuyer() {
    ...
    for(var i in this.buyList){
    this.buyList[i].image=this.buyList[i].image.split(",");
    this.storageList.push(this.buyList[i]);
    }
    if (res.status === 0) {
