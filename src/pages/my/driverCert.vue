...
<template>
  <view class="container">
   <form bindsubmit='submit'>
         <view class="section">
        <text class="sec_title">请上传您的司机驾驶证</text>
            <view class="upload_content" @tap="testbtn()" data-index="1">
         <view class="icon_box"  hidden="{{driver_licence[0]}}">
          <view></view>
         <view></view>
        </view>
        <image src="{{driver_licence[0]}}"   mode="aspectFill"  />
        </view>
  </view>

       <view class="section">
        <text class="sec_title">请上传您的车辆驾驶证</text>
            <view class="upload_content" @tap="testbtn()" data-index="2">
         <view class="icon_box"  hidden="{{car_licence[0]}}">
          <view></view>
         <view></view>
        </view>
        <image src="{{car_licence[0]}}"   mode="aspectFill"  />
        </view>
  </view>
        
  <view class="section">
        <text class="sec_title">身份认证：上传身份证正反面两面照片</text>
            <view class="upload_content" @tap="testbtn()" data-index="3">
   ...
   <view class="upload_content" @tap="testbtn()" data-index="4">
...
export default class DriverCert extends wepy.page {
  config = {
  ...
  async upImg(imgurls,v) {
    ...
    if(v==1){
        this.driver_licence = await uploadSeriesFile(imgurls);
    }else if (v==2){
        this.car_licence = await uploadSeriesFile(imgurls);
    }else if (v==3){
  ...
  async postForm() {
    const res = await shttp
      .post(`/api/v1/driver/cert`)
      .send({
          driver_licence:this.driver_licence[0],
          car_licence:this.car_licence[0],
        driver_name: this.name,
        front_idcard: this.front[0],
        back_idcard: this.back[0],
        driver_phone: this.telephone
      })
      .end();
    wx.hideLoading();
    if (res.status == 0) {
