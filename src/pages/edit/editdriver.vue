  ...
  <view class="container">
   <form bindsubmit='submit'>
       ...
       <view class="row_box">
           ...
           <view class="row_control">
               <picker mode="region"  bindchange="getAddrs" value="{{addrs}}" custom-item='不限'>
             <view class="picker">
              {{area}}
                </view>
                 <image class="icon_down" src="../../images/down@2x.png" />
...
export default class EditDriver extends wepy.page {
  config = {
    navigationBarTitleText: "修改找货"
  };
  data = {
    //分类
    classify: {
  ...
  methods = {
   bindRegionChange: e => (this.city = e.detail.value,this.address=this.city[1] + this.city[2]),
    ...
    getPigcage: e => (this.pigCage = parseInt(e.detail.value) + 1),
     ...
     getAddrs: e =>{
       this.addrs = e.detail.value;
      // console.log(this.city)
      if(this.addrs[0]=="不限"){
        this.area="不限"
      }
      else if(this.addrs[1]=="不限"){
        this.area=this.addrs[0]
      }else if(this.addrs[2]=="不限"){
          this.area=this.addrs[1]
      }
      else{
        this.area=this.addrs[1]+this.addrs[2];
      }
    },
